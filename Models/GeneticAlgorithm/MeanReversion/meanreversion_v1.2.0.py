...
df_big = dl.get(Constants.GBPUSD, Constants.DAILY, start=start, end=end)
df_small = dl.get(Constants.GBPUSD, Constants.FIVE_MINUTES, start=start, end=end)

# Add Offset
df_big.index = df_big.index + dl.getPeriodOffsetSeconds(Constants.DAILY)
...
class GeneticPlanModel(GA.GeneticAlgorithmModel):
	...
	def __call__(self, X, y, training=False):
		...
		if training:
			self.tp = min(max(out[0,0][2], 30.0), 250.0)
	...
	def getOutput(self, X):
		...
		x[:,:,2] = np.round(x[:,:,2] * ((250.0-30.0) + 30.0))
	...
	def getPerformance(self, ret, dd, gain, loss, wins, losses, s_ret, s_dd, training=False):
		...
		if training:
			ret_mod = s_ret
			dd_mod = pow(max(s_dd-8, 0), 2)
		else:
			ret_mod = ret
			dd_mod = pow(max(dd-8, 0), 2)
