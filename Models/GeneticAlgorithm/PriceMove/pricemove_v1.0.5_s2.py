...
class LoadModel(object):
	...
	def __call__(self, X):
		return self.getOutput(cp.asarray(X))
...
model_name = 'v1.0.5_3m_30c_4hl/0'
with open('./saved/{}.json'.format(model_name), 'r') as f:
	info = json.load(f)
	mean = info['mean']
	std = info['std']
...
# m_train = model(X_train_norm)
# m_val = model(X_val_norm)
# print(m_train[:5])
# print(m_train.shape)
# print(m_val[:5])
# print(m_val.shape)
...
class GeneticPlanModel(GA.GeneticAlgorithmModel):
	...
	def getPerformance(self, ret, dd, gain, loss, wins, losses, training=False):
		...
		return ret - trades_mod

	def generateModel(self, model_info):
		return [
			GA.RNN_TWO_1D_GPU(2, 2),
		]
	...
	def __str__(self):
		return  ' (Train) Perf: {:.2f}\tWins: {:.0f}\tLosses: {:.0f}\n' \
				'   (Val) Perf: {:.2f}\tWins: {:.0f}\tLosses: {:.0f}\n'.format(
			self.getPerformance(*self.train_results, training=True), self.train_results[4], self.train_results[5],
			self.getPerformance(*self.val_results), self.val_results[4], self.val_results[5],
		)
			...
			if bt.get_num_positions(positions) < 5:
				if out[i][0] > out[i][1]:
					if out[i][0] >= threshold:
						positions = bt.create_position(positions, charts[j][i], bt.BUY, max_low, max_high, max_low)
				else:
					if out[i][1] >= threshold:
						positions = bt.create_position(positions, charts[j][i], bt.SELL, max_low, max_high, max_low)
...
model = GeneticPlanModel(X_train_plan, X_val_plan, threshold=0.5)
with open('./saved/{}.json'.format(model_name), 'r') as f:
	info = json.load(f)
	weights = [np.array(i, dtype=np.float32) for i in info['weights']]

model.setModel(model.generateModel(None))
model.setWeights(weights)

print(model(cp.asarray(X_train_norm), y_train, training=True))
print(model(cp.asarray(X_val_norm), y_val))
print(model)
...
# # ga.setSeed(1)
# ga.saveBestTrain(10, 'v1.0.5_6m_60c', {'mean': float(mean), 'std': float(std)})
