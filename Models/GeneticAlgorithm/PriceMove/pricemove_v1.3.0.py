...
df_m = dl.get(Constants.GBPUSD, Constants.TEN_MINUTES, start=start, end=end)
...
class GeneticPlanModel(GA.GeneticAlgorithmModel):
	...
	def __call__(self, X, y, training=False):
		...
		if training:
			...
			self.tp = min(max(out[0][3], 5.0), 50.0)
	...
	def getOutput(self, X):
		...
		x[:,3] = np.round(x[:,3] * ((50.0-5.0) + 5.0), decimals=2)
	...
	def getPerformance(self, ret, dd, gain, loss, wins, losses, training=False):
		...
		if training:
			min_trades = (num_months * 60) * data_split
		else:
			min_trades = (num_months * 60) * (1 - data_split)
