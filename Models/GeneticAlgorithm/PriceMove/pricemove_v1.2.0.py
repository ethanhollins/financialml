...
class GeneticPlanModel(GA.GeneticAlgorithmModel):
	...
	def __call__(self, X, y, training=False):
		...
		if training:
			...
			results, data = bt.start(
				GeneticPlanModel.run, y.astype(np.float32), self.threshold,
				out, self.X_val_plan, self.sl, self.tp, self.risk
			)
			results = [
				results[0], # Return
				results[4], # Drawdown
	...
	def getOutput(self, X):
		...
		x[:,2] = np.round(x[:,2] * ((50.0-10.0) + 10.0))
		...
		x[:,3] = np.round(x[:,3] * ((5.0-2.0) + 2.0), decimals=2)
	...
	def getPerformance(self, ret, dd, gain, loss, wins, losses, training=False):
		...
		if training:
			min_trades = (num_months * 200) * data_split
		else:
			min_trades = (num_months * 200) * (1 - data_split)
			...
			if bt.get_num_positions(positions) < 5:
				if out[i][0] > out[i][1]:
					if out[i][0] >= threshold:
						positions = bt.create_position(positions, charts[j][i], bt.BUY, sl, _tp, sl/risk)
				else:
					if out[i][1] >= threshold:
						positions = bt.create_position(positions, charts[j][i], bt.SELL, sl, _tp, sl/risk)
