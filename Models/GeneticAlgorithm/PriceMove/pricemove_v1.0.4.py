...
class GeneticPlanModel(GA.GeneticAlgorithmModel):
	...
	def __call__(self, X, y, training=False):
		...
		if training:
			...
			self.train_results = data[:4]
		else:
			_, data = bt.start(
				GeneticPlanModel.run, y.astype(np.float32), self.threshold,
				out, self.X_val_plan
			)
			self.val_results = data[:4]

		return self.getPerformance(data[:4], training=training)
	...
	def __str__(self):
		return  ' (Train) Perf: {:.2f}  Wins: {:.0f}  P Wins: {:.0f}  Losses: {:.0f}\n' \
				'   (Val) Perf: {:.2f}  Wins: {:.0f}  P Wins: {:.0f}  Losses: {:.0f}\n'.format(
			self.getPerformance(self.train_results), self.train_results[1], self.train_results[2], self.train_results[3],
			self.getPerformance(self.val_results), self.val_results[1], self.val_results[2], self.val_results[3]
		)
		...
		for x in range(bt.get_num_positions(positions)-1,-1,-1):
			...
			if dist_min > c_min:
				if dist_min > max_low:
					...
					data[4+(x*3):-3] = data[4+((x+1)*3):]
					data[-3:] = 0
					continue

				else:
					data[4+(x*3+2)] = dist_min
			
			if dist_max > c_max:
				data[4+(x*3+1)] = data[4+(x*3+2)]

				if dist_max > max_high:
					data[0] += (max_high - data[4+(x*3+1)])
					data[1] += 1
					positions, result = bt.close_position(positions, x, charts[j][i], result, stats)
					data[4+(x*3):-3] = data[4+((x+1)*3):]
					data[-3:] = 0
				...
				if out[i][0] > out[i][1]:
					if out[i][0] >= threshold:
						if c_dir == bt.SELL:
							...
							positions, result = bt.stop_and_reverse(positions, charts[j][i], result, stats, bt.BUY, 999, 0, 999)
							data[4+(x*3):-3] = data[4+((x+1)*3):]
							data[-3:] = 0
					...
					if out[i][1] >= threshold:
						if c_dir == bt.BUY:
							...
							positions, result = bt.stop_and_reverse(positions, charts[j][i], result, stats, bt.SELL, 999, 0, 999)
							data[4+(x*3):-3] = data[4+((x+1)*3):]
							data[-3:] = 0
...
# ga.setSeed(1)
# ga.saveBestTrain(10, 'v1.0.3_3m', {'mean': float(mean), 'std': float(std)})
