...
class GeneticPlanModel(GA.GeneticAlgorithmModel):
	...
	def __call__(self, X, y, training=False):
		...
		if training:
			self.tp = min(max(out[0][3], 15.0), 80.0)
	...
	def getOutput(self, X):
		...
		x[:,2] = np.round(x[:,2] * ((80.0-15.0) + 15.0))
		...
		x[:,3] = np.round(x[:,3] * ((80.0-15.0) + 15.0), decimals=2)
