...
def getTrainData(data, timestamps):
	...
	X_out = []
	X_plan = []
	c_data = np.zeros((data_points,3), dtype=np.float32)
	c_plan = np.zeros((3,))
...
class GeneticPlanModel(GA.GeneticAlgorithmModel):
	...
	def getOutput(self, X):
		...
		x[:,2] = np.round(x[:,2] * ((80.0-10.0) + 10.0))
		...
		x[:,3] = np.round(x[:,3] * ((80.0-5.0) + 5.0), decimals=2)
		...
		x[:,4] = np.round(x[:,4] * ((2.0-0.1) + 0.1), decimals=2)
	...
	def getPerformance(self, ret, dd, gain, loss, wins, losses, training=False):
		...
		if training:
			min_trades = (num_months * 5) * data_split
		else:
			min_trades = (num_months * 5) * (1 - data_split)
	...
	@jit
	def run(i, j, positions, charts, result, data, stats, threshold, out, plan, risk):
		sl = min(max(out[i][2], 10.0), 80.0)
		tp = min(max(out[i][3], 5.0), 80.0)
