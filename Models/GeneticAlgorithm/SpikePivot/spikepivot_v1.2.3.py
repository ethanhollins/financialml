...
def getTrainData(data):
	...
	for i in range(lookback, data.shape[0]):
		...
		if c_data[1]:
			c_data[1] = convertToPips((ad_data[0] - data[i,3]) * (ad_data[0] / ad_data[1]))
		...
		if isLongSpike(
			...
			c_data[0] = convertToPips((data[i,3] / ad_data[1]) - (ad_data[0] / ad_data[1]))
		...
		if isShortSpike(
			...
			c_data[1] = convertToPips((ad_data[3] / ad_data[2]) - (ad_data[3] / data[i,3]))
...
class GeneticPlanModel(GA.GeneticAlgorithmModel):
	...
	def getPerformance(self, ret, dd, gain, loss):
		if loss == 0:
			...
			gpr = (gain / loss) + 1
		return (ret * min(2, gpr)) - pow(dd, 2)

	def generateModel(self, model_info):
		return BasicDenseModel(1, [16, 16, 1])
	...
	@jit(forceobj=True)
	def run(i, positions, ohlc, result, data, threshold, inpt, W1, W2, W3, b1, b2, b3):
		c_dir = bt.get_direction(positions, 0)
		sl = 100.0

		if c_dir == bt.BUY:
			out = model_run(inpt[i][1:2], W1, W2, W3, b1, b2, b3)
			if out[0] > threshold:
				positions, result = bt.stop_and_reverse(positions, ohlc[i], result, bt.SELL, 0, 0, sl)

		elif c_dir == bt.SELL:
			out = model_run(inpt[i][0:1], W1, W2, W3, b1, b2, b3)
			if out[0] > threshold:
				positions, result = bt.stop_and_reverse(positions, ohlc[i], result, bt.BUY, 0, 0, sl)

		else:
			out_one = model_run(inpt[i][1:2], W1, W2, W3, b1, b2, b3)
			out_two = model_run(inpt[i][0:1], W1, W2, W3, b1, b2, b3)
			if out_one[0] > out_two[0]:
				if out_one[0] > threshold:
					positions = bt.create_position(positions, ohlc[i], bt.SELL, 0, 0, sl)

			else:
				if out_two[0] > threshold:
					positions = bt.create_position(positions, ohlc[i], bt.BUY, 0, 0, sl)
