...
df = dl.get(Constants.GBPUSD, Constants.FOUR_HOURS, start=dt.datetime(2019,1,1))
...
class GeneticPlanModel(GA.GeneticAlgorithmModel):
	...
	def __call__(self, X, y, training=False):
		super().__call__(X, y)
		results = bt.start(
			GeneticPlanModel.run, y.astype(np.float32), X
		)
		results = [
			results[0], # Return
			results[4], # Drawdown
	...
	@jit
	def run(i, positions, ohlc, result, data, out):
		# print(out[i])
		# print(ohlc[i])
		sl = 55.0
		if out[i][0] == 1 and out[i][1] < 200.0:
			c_dir = bt.get_direction(positions, 0)
			# sl = min(max(out[i][1]+1.0, 30.0), 55.0)
			if not c_dir:
				# print('Buy')
		...
		elif out[i][2] == 1 and out[i][3] < 200.0:
			c_dir = bt.get_direction(positions, 0)
			# sl = min(max(out[i][3]+1.0, 30.0), 55.0)
			if not c_dir:
				# print('Sell')
		...
		# 	if pos_sl > 0 and profit >= pos_sl*2:
		# 		positions = bt.modify_sl(positions, 0, -sl)
