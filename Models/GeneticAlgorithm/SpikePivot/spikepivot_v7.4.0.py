...
class GeneticPlanModel(GA.GeneticAlgorithmModel):
	...
	def __call__(self, X, y, training=False):
		...
		if training:
			self.sl = min(max(out[0,0][2], 30.0), 150.0)
			self.tp = min(max(out[0,0][3], 15.0), 150.0)
	...
	def getOutput(self, X):
		...
		x[:,:,3] = np.round(x[:,:,3] * ((150.0-30.0) + 30.0), decimals=2)
	...
	def generateModel(self, model_info):
		return [
			GA.Conv2D_GPU(kernel_shape=(3,3), row_stride=2, col_stride=1),
