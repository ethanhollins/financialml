...
class GeneticPlanModel(GA.GeneticAlgorithmModel):
	...
	def __call__(self, X, y, training=False):
		...
		if training:
			...
			self.max_trades = round(min(max(out[0][4], 1.0), 3.0))
			results, data = bt.start(
				GeneticPlanModel.run, y.astype(np.float32), self.threshold,
				out, self.X_train_plan, self.tp, self.max_trades
			)
			results = [
				results[0], # Return
				results[4], # Drawdown
			...
			results, data = bt.start(
				GeneticPlanModel.run, y.astype(np.float32), self.threshold,
				out, self.X_val_plan, self.tp, self.max_trades
			)
			results = [
				results[0], # Return
				results[4], # Drawdown
	...
	def getOutput(self, X):
		...
		x[:,4] = np.round(x[:,4] * ((3.0-1.0) + 1.0), decimals=2)
	...
	def getPerformance(self, ret, dd, gain, loss, wins, losses, training=False):
		...
		if training:
			min_trades = (num_months * 4) * data_split
		else:
			min_trades = (num_months * 4) * (1 - data_split)
	...
	@jit
	def run(i, j, positions, charts, result, data, stats, threshold, out, plan, tp, max_trades):
		# Misc variables
		risk = 1.0
