...
df_m = dl.get(Constants.GBPUSD, Constants.FIFTEEN_MINUTES, start=start, end=end)
...
class GeneticPlanModel(GA.GeneticAlgorithmModel):
	...
	def __call__(self, X, y, training=False):
		...
		if training:
			self.sl = min(max(out[0][2], 30.0), 150.0)
			self.tp = min(max(out[0][3], 15.0), 150.0)
	...
	def getOutput(self, X):
		...
		x[:,2] = np.round(x[:,2] * ((150.0-30.0) + 30.0))
		...
		x[:,3] = np.round(x[:,3] * ((150.0-15.0) + 15.0), decimals=2)
	...
	def run(i, j, positions, charts, result, data, stats, threshold, out, plan, sl, tp):
		...
		multi = 0.5 * (i / charts.shape[1]) + 0.5
