...
mean = 0
std = np.std(X_train[:, [1,3]])

X_train_norm = np.copy(X_train)
X_train_norm[:, [1,3]] = normalize(X_train[:, [1,3]], mean, std)
X_val_norm = np.copy(X_val)
X_val_norm[:, [1,3]] = normalize(X_val[:, [1,3]], mean, std)
...
class GeneticPlanModel(GA.GeneticAlgorithmModel):
	...
	def generateModel(self, model_info):
		return BasicDenseModel(4, [32, 32, 4])
	...
	def run(i, positions, ohlc, result, data, threshold, out):
		...
		if c_dir == bt.BUY:
			if out[i][0] > threshold:
				positions, result = bt.stop_and_reverse(positions, ohlc[i], result, bt.SELL, 999, 0, sl)
			elif out[i][3] > threshold:
				positions, result = bt.close_position(positions, 0, ohlc[i], result)

		elif c_dir == bt.SELL:
			if out[i][1] > threshold:
				positions, result = bt.stop_and_reverse(positions, ohlc[i], result, bt.BUY, -999, 0, sl)
			...
			if out[i][0] > out[i][1]:
				if out[i][0] > threshold:
					positions = bt.create_position(positions, ohlc[i], bt.SELL, 999, 0, sl)

			else:
				if out[i][1] > threshold:
					positions = bt.create_position(positions, ohlc[i], bt.BUY, -999, 0, sl)
...
ga.setSeed(1)
ga.saveBest(10, 'v1.6.1_30m_010118', {'mean': float(mean), 'std': float(std)})
