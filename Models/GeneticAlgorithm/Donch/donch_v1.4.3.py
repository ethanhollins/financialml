...
def getDonchUpDown(high, low, period):
	...
	for i in range(period, high.shape[0]):
		...
		if last_high != 0 and last_low != 0:
			...
			X.append(x)
			if not validation:
				validation = [high[i], low[i]]
		last_high = c_high
		last_low = c_low

	return np.array(X, dtype=np.float32), validation

period = 4
timer = timeit()
train_data, validation = getDonchUpDown(
	df.values[:,1],
	df.values[:,2],
	period
)
...
print('\nCheck validation:')
print('{}, {}'.format(round(float(y_train[0][1]), 5), validation[0]))
print('{}, {}'.format(round(float(y_train[0][2]), 5), validation[1]))
assert round(float(y_train[0][1]), 5) == validation[0]
assert round(float(y_train[0][2]), 5) == validation[1]
...
class GeneticPlanModel(GA.GeneticAlgorithmModel):
	def __init__(self, in_size, layers, max_pos=2, threshold=0.5):
		super().__init__()
	...
	def generateModel(self, model_info):
		return BasicDenseModel(self.in_size, self.layers)

	def newModel(self):
		return GeneticPlanModel(
			self.in_size, self.layers, self.max_pos, self.threshold
		)
	...
	def run(i, positions, ohlc, result, data, out, threshold, max_pos):
		...
		if out[i][1] > (1 - threshold):
			...
			if not c_dir:
				positions = bt.create_position(positions, ohlc[i], bt.BUY, sl, 0)
			elif c_dir == bt.BUY:
		...
		if out[i][0] < threshold:
			c_dir = bt.get_direction(positions, 0)
			if not c_dir:
				positions = bt.create_position(positions, ohlc[i], bt.SELL, sl, 0)
			elif c_dir == bt.SELL:
				if bt.get_num_positions(positions) < max_pos:
...
def generate_models(num_models):
	...
	for i in range(num_models):
		models.append(
			GeneticPlanModel(
				2, [32,32,4], max_pos=1, threshold=0.5
			)
		)
	return models
