...
class GeneticPlanModel(GA.GeneticAlgorithmModel):
	def __init__(self, sl, tp, max_pos=2, threshold=0.5):
		super().__init__()
		self.sl = sl
		self.tp = tp
	...
	def __call__(self, X, y, training=False):
		super().__call__(X, y)
		results = bt.start(
			GeneticPlanModel.run, y.astype(np.float32), 
			self._model(X), self.threshold, self.sl, self.tp, self.max_pos
		)
		results = [
			results[0], # Return
			results[4], # Drawdown
	...
	def newModel(self):
		return GeneticPlanModel(self.sl, self.tp, self.max_pos, self.threshold)
	...
	@jit
	def run(i, positions, ohlc, result, data, out, threshold, sl, tp, max_pos):
		# Long Entry
		if out[i][1] > (1 - threshold):
			...
			if not c_dir:
				...
				if bt.get_num_positions(positions) < max_pos:
					positions = bt.create_position(positions, ohlc[i], bt.BUY, sl, 130.)
			elif c_dir == bt.SELL:
		...
		if out[i][0] < threshold:
			...
			if not c_dir:
				...
				if bt.get_num_positions(positions) < max_pos:
					positions = bt.create_position(positions, ohlc[i], bt.SELL, sl, 130.)
			elif c_dir == bt.BUY:
...
def generate_models(num_models):
	...
	for i in range(num_models):
		models.append(GeneticPlanModel(130., 0., 2, threshold=0.5))
	return models
