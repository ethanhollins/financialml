...
train_data = getDonchUpDown(
	df.loc[df.index <= end_ts].values[:,5],
	df.loc[df.index <= end_ts].values[:,6],
	period,
	2 # lookup
)
test_data = getDonchUpDown(
	df.loc[df.index > end_ts].values[:,5],
	df.loc[df.index > end_ts].values[:,6],
	period,
	2 # lookup
...
train_size = int(df.loc[df.index <= end_ts].shape[0] * 0.7)
train_period_off = df.loc[df.index <= end_ts].shape[0] - train_data.shape[0]
test_period_off = df.loc[df.index > end_ts].shape[0] - train_data.shape[0]

X_train = train_data[:train_size]
y_train = df.loc[df.index <= end_ts].values[train_period_off:train_size+train_period_off].astype(np.float32)
X_val = train_data[train_size:]
y_val = df.loc[df.index <= end_ts].values[train_size+train_period_off:].astype(np.float32)
...
class GeneticPlanModel(GA.GeneticAlgorithmModel):
	...
	def __str__(self):
		return  ' (Train) Perf: {:.2f}\t% Ret: {:.2f}%\t% DD: {:.2f}%\tGPR: {:.2f}\n' \
				...
				'  (Test) Perf: {:.2f}\t% Ret: {:.2f}%\t% DD: {:.2f}%\tGPR: {:.2f}\n'.format(
			...
			(self.val_results[2] / self.val_results[3]) if self.test_results[3] != 0 else 0,
...
for i in range(10):
	...
	with open('./saved/v1.7.0_1_old/{}.json'.format(i), 'r') as f:
		info = json.load(f)
	...
	print(gpm(X_train, y_train, mode=0))
	print(gpm(X_val, y_val, mode=1))
