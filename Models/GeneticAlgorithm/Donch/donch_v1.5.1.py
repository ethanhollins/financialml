...
df = dl.get(Constants.GBPUSD, Constants.THIRTY_MINUTES, start=dt.datetime(2019,5,1), end=dt.datetime(2019,11,1))
...
def getDonchUpDown(high, low, period):
	...
	for i in range(period, high.shape[0]):
		...
		if last_high and last_low:
			...
			x.append(convertToPips((c_low - last_low)/last_low))
