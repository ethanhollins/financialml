...
def getTrainData(data_big, data_small, timestamps, lookup):
	...
	for t_i in range(lookup, timestamps.size-pred, step):
		...
		if data_big[t_i+pred,3] >= data_big[t_i,3]:
			c_plan[0] = bt.BUY
		else:
			c_plan[0] = bt.SELL
...
class GeneticPlanModel(GA.GeneticAlgorithmModel):
	...
	def __call__(self, X, y, training=False):
		...
		if training:
			...
			self.val_results = data[:3]

		return self.getPerformance(*data[:3], training=training)
	...
	def getPerformance(self, amount, correct, incorrect, training=False):
	...
	def __str__(self):
		return  '  (Train) (P) {:.2f} Corr: {:.0f} Inc: {:.0f} Amt: {:.0f}\n' \
				'    (Val) (P) {:.2f} Corr: {:.0f} Inc: {:.0f} Amt: {:.0f}\n'.format(
			self.getPerformance(*self.train_results, training=True), 
			self.train_results[1], self.train_results[2], self.train_results[0],
			self.getPerformance(*self.val_results), 
			self.val_results[1], self.val_results[2], self.val_results[0], 
		)
